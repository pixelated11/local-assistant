...
label_title = ctk.CTkLabel(app, text="LocalAssistant", font=ctk.CTkFont("Monospace", 35))
label_title.grid(row=0, column=0, padx=10, pady=10)

label_title_desc = ctk.CTkLabel(app, text="Your local AI application, powered with Qwen3 LLM.", font=ctk.CTkFont("Calibri", 15))
label_title_desc.grid(row=1, column=0, padx=35, pady=3)

label_response = ctk.CTkLabel(app, text="AI's Response: ", font=ctk.CTkFont("Calibri", 12))
...
# Send button
button_send = ctk.CTkButton(app, text="Send", command=send_prompt, font=ctk.CTkFont("Calibri", 15), height=45)
button_send.grid(row=5, column=0, padx=10, pady=5, sticky="ew")

# Status label
label_status = ctk.CTkLabel(app, text="● AI Status: Idle", font=ctk.CTkFont("Calibri", 13))
...
button_stop = ctk.CTkButton(app, text="Stop AI (If you don't, it will run in the background, and consume RAM.)", command=stop_ai, font=ctk.CTkFont("Calibri", 15))
